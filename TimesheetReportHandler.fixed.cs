    ...
    public class TimesheetReportHandler : IRequestHandler<GetProjectReportQuery, List<ProjectReportRow>>
    {
        ...
        public async Task<List<ProjectReportRow>> Handle(GetProjectReportQuery request, CancellationToken token)
        {
            ...
            var monthEntries = await _db.GetCollection<TimeEntry>("time_entries")
                .Find(e => e.Date >= start && e.Date < end)
                .ToListAsync(token);

            var employeeIds = monthEntries.Select(e => e.EmployeeId).ToHashSet();
            var projectIds = monthEntries.Select(e => e.ProjectId).ToHashSet();
            ...
            {
                if (!employeeById.TryGetValue(entry.EmployeeId, out var employee))
                {
                    continue;
                }
                ...
                {
                    if (!projectById.TryGetValue(entry.ProjectId, out var project))
                    {
                        continue;
                    }
